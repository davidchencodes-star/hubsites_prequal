...
export async function POST(request: NextRequest) {
  try {
    ...
    logit('API Data Request:', { id, deployType, parentDomain, parentIP })
    ...
    logit('API Response Status:', response.status)

    const data = await response.json();
    logit('API Response Data:', data)
